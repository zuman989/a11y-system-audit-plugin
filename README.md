...
CheckWhat it catchesContrastText failing WCAG AA ratios (4.5:1 normal, 3:1 large text)Focus stateFocus variants that look identical to default — invisible to keyboard usersFont sizeText below 12pxTouch targetInteractive elements under 24x24px (WCAG 2.2)
...
Open the plugin in any Figma file that has component sets
Click Run audit — it reads the component sets directly, no manual selection or frame-wrapping needed
Issues are listed by severity (High / Medium / Low), click any issue to jump straight to that layer in Figma
Optionally click Generate stakeholder explanations to get plain-language summaries via Groq (free API key, no credit card)
Export the full list as a .md remediation plan to share with your team
...
View the test design system on Figma
...
Full write-up on the problem, the debugging process, and design decisions on Behance.
